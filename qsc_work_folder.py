...
def normalize_ci_title(poem: Dict) -> str:
    """宋词数据使用 rhythmic 作为标题，若无则回退到 title"""
    return poem.get("rhythmic") or poem.get("title", "")
...
def get_chunk_file_path(base_path: str, chunk_index: int) -> str:
    """
    根据分片索引生成分片文件路径
    chunk_index 0: ci.song.1000.json (基础文件，保存 0~199)
    chunk_index 1: ci.song.1000.1.json (保存 200~399)
    chunk_index 2: ci.song.1000.2.json (保存 400~599)
    """
    if chunk_index == 0:
        return base_path
    # 移除 .json 后缀，添加分片编号
...
def process_single_file(file_path: str) -> bool:
    ...
    - ci.song.1000.json 保存第 0~199 首
    - ci.song.1000.1.json 保存第 200~399 首
    - ci.song.1000.2.json 保存第 400~599 首
    """
    ...
    with file_lock:
        ...
        print(f"🚀 [{file_name}] 开始/继续处理，共 {num_poems} 首词，已处理 {processed_count} 首。")
        ...
        for i in range(processed_count, num_poems, BATCH_SIZE):
            ...
            # 提取 API 需要的字段（宋词使用 rhythmic 作为标题）
            batch_to_send = []
            for p in batch:
                batch_to_send.append({
                    "title": normalize_ci_title(p),
                    "author": p.get("author", ""),
        ...
        print(f"✅ [{file_name}] 处理完成！共 {processed_count} 首词。\n")
        update_file_progress(file_name, processed_count, num_poems, "completed")
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="宋词处理脚本 - 从宋词文件夹读取 JSON 文件，处理后保存到指定输出文件夹",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python qsc_work_folder.py
  python qsc_work_folder.py -i ./宋词 -o ./宋词clean
  python qsc_work_folder.py -i ./宋词 -o ./宋词clean --pattern "ci.song.*.json"
        """
    )
    parser.add_argument(
        "-i", "--input",
        default="宋词",
        help="输入文件夹路径，包含待处理的宋词 JSON 文件（默认: ./宋词）"
    )
    parser.add_argument(
        "-o", "--output",
        default="宋词clean",
        help="输出文件夹路径，处理后的文件将保存在此（默认: ./宋词clean）"
    )
    parser.add_argument(
        "-p", "--pattern",
        default="ci.song.*.json",
        help="文件匹配模式，默认为 'ci.song.*.json'"
    )
    parser.add_argument(
        "-w", "--workers",
        type=int,
    ...
    for file_path in files:
        file_name = os.path.basename(file_path)
        if "_analyzed" in file_name:
            continue
        filtered_files.append(file_path)
    files = filtered_files

    if not files:
        print(f"未找到可处理的文件（已过滤 *_analyzed.json）: {source_pattern}")
        return
